...
class Table:
    ...
    def edit_arr(self,
        ...
        for row in range(
                max([len(content_arr), len(length_arr), len(align_arr), len(cell_type_arr), len(cell_format_arr)])):
            for column in range(max([len(content_arr[0]), len(length_arr[0]), len(align_arr[0]), len(cell_type_arr[0]),
    ...
    def str_to_center(self, string_, length, filling=" "):
        return filling[0] * math.ceil((length - len(string_)) / 2) + string_ + filling[0] * math.floor(
            (length - len(string_)) / 2)
